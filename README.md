...
- **通行密钥（WebAuthn）登录**：用指纹/Face ID/PIN/安全密钥免密登录，支持多设备
- 上传进度实时显示
- 图片画廊浏览已上传图片
...
除了密码登录，本服务还支持用**通行密钥**免密登录（指纹 / Face ID / 设备 PIN / USB 安全密钥）。

### 使用方式

1. **先登录一次**（用密码），进入「通行密钥」管理页（顶部导航最右侧）
2. 点击「注册新通行密钥」，浏览器弹出系统认证，完成即可
3. 之后在登录页点击「使用通行密钥登录」，无需输入密码
4. **建议至少注册 2 个设备**作为备份，避免丢失设备后无法登录

### 注意事项

- **域名绑定**：通行密钥与 `BASE_IMG_URL` 的 hostname（RP ID）绑定。**更换域名后所有已注册的通行密钥将失效**（密码登录不受影响，仍可作兜底重新注册）。
- **HTTPS 要求**：WebAuthn 仅在 HTTPS 或 `localhost` 下可用（EdgeOne 默认 HTTPS，本地 dev 走 localhost 放行）。
- **密码登录保留**：通行密钥是可选增强，不会替代密码登录，二者并行可用。
- **多设备**：每个通行密钥是独立的一条记录，可在管理页查看 / 命名 / 删除。

详见 [机制原理 - 通行密钥](./docs/MECHANISM.md#通行密钥-webauthn) 与 [API - 通行密钥端点](./docs/API.md#通行密钥端点)。
