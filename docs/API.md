...
> **首次部署初始化**（自动一次性触发）：部署后第一个访问受保护页（如 `/home`）的用户会被守卫自动跳转到生成的登录路径。之后该端点变 403。也可手动 `curl https://your-domain.com/api/auth/login-path`（首次公开）立即初始化。详见 [MECHANISM.md 首次初始化流程](./MECHANISM.md#首次初始化流程部署后自动触发一次)。
